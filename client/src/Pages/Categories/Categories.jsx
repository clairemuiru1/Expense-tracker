...
function Categories() {
    ...
    const lightColors = ['#f9d5e5', '#f9edd2', '#d2f9f2', '#d2d2f9', '#f2d2f9', '#d2f9d2'];
        ...
        <div className="categories-container">
            ...
            <ul className="category-list">
                {categories.map((category, index) => {
                    const categoryBudget = budgets.find(budget => budget.category.name === category.name);
                    return (
                        <li key={category.id} style={{ backgroundColor: lightColors[index % lightColors.length] }}>
                            <h3>{category.name}</h3>
                            {categoryBudget && (
